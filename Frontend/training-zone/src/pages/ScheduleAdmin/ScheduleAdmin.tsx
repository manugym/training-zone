...
export default function ScheduleAdmin() {
  ...
  const handleStartDateChange = (date: Date | null) => {
    if (date) setForm((f) => ({ ...f, StartDateTime: date }));
  };

  const handleEndDateChange = (date: Date | null) => {
    if (date) setForm((f) => ({ ...f, EndDateTime: date }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId === null) {
        ...
        const updateData: UpdateSchedule = { ...form };
                  ...
                  <tr key={item.Id}>
                    ...
                    <td>
                      {(() => {
                        ...
                        return cls ? `${ClassType[cls.Type]}` : item.ClassId;
                      })()}
                    </td>
                    <td>
                      {(() => {
                        const foundTrainer = trainers.find(
                          (t) => t.Id === item.Trainer.Id
                        );
                        return (
                          foundTrainer
                            ? foundTrainer.Name
          ...
          <div className="modal-backdrop">
            <div className="modal-content">
              <form onSubmit={handleSubmit} className="modal-form">
                <h2>
                  {editingId === null ? "Crear Horario" : "Editar Horario"}
                </h2>
